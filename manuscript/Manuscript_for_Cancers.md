...
**Target journal:** *Cancers* (MDPI; Section: *Cancer Therapy*; ISSN 2072-6694)
...
\* **Correspondence:** sheikh.islam@kcl.ac.uk (S.K.I.); alternative: kamrul0405@outlook.com.

---

## Simple Summary

Brain metastases from primary cancers — most commonly lung, breast and melanoma — are treated with stereotactic radiotherapy (SRS), a high-precision, single- or short-course radiation delivery technique. Despite optimal targeting, 30–40% of recurrent tumour volume falls outside the planned high-dose region. Identifying the *spatial* regions where future recurrences are most likely could in principle support adaptive re-treatment or earlier surveillance — but the most common spatial-prediction baseline (a smoothed expansion of the baseline tumour mask) has not been benchmarked directly against the actual delivered radiation dose distribution. Using a public dataset of 43 brain-metastasis SRS patients with patient-specific delivered-dose NIfTI files, we show that a closed-form spatial smoothing of the baseline tumour mask captures, on average, 47% of future-lesion volume — about 10 percentage points more than the prescription-dose envelope alone. This advantage holds in both single-fraction and seven-fraction treatment groups. The framework is fully reproducible and provides a benchmark against which future learned voxel-wise spatial-prediction methods can be evaluated. The analysis cannot quantify toxicity costs, so it does not directly support dose escalation; it is intended as retrospective audit infrastructure and motivation for prospective trial design.

---

## Abstract

**Simple background.** Brain-metastasis stereotactic radiotherapy (SRS) is followed by spatial recurrence that can fall outside the planned dose envelope. A closed-form spatial smoothing of the baseline lesion mask (a heat-kernel structural prior) may indicate peri-lesional regions of elevated future-lesion risk and could complement dosimetric coverage as an audit tool. **Methods.** We analysed 43 brain-metastasis SRS patients (122 follow-up paired evaluations) from the publicly released PROTEAS-brain-mets dataset (Zenodo doi:10.5281/zenodo.17253793). Treatment was delivered as single-fraction (n = 93 rows; 18 or 20 Gy in 33 patients) or 7-fraction hypofractionated SRT (n = 28 rows; 35 Gy = 5 Gy × 7 in 9 patients). Patient-specific RTDOSE, baseline and follow-up lesion masks were co-registered. Future-lesion coverage by dose ≥ 95% Rx, dose ≥ 100% Rx, heat ≥ 0.50 and heat ≥ 0.80 was computed (heat = Gaussian convolution of the baseline mask, σ = 2.5 voxels). Cluster bootstrap (10,000 patient-level replicates) gave 95% CIs. Pre-specified primary endpoints used Holm–Bonferroni step-down at FWER = 0.05. A 5 × 6 threshold-sensitivity sweep and fractionation-stratified secondary analysis were performed. **Results.** Pooled coverage: dose ≥ 95% Rx 37.8% (CI 27.8–48.2); heat ≥ 0.50 47.3% (CI 37.4–57.2); heat ≥ 0.80 30.1% (CI 22.1–38.5). Heat ≥ 0.50 exceeded dose ≥ 95% Rx by +9.5 percentage points (Wilcoxon p = 0.014; row-level failure rate 52.1% vs 60.3%, p < 0.05 step-down). Fractionation-stratified advantage was preserved (single-fraction +11.3 pp; 7-fraction +2.8 pp). Heat ≥ 0.80 V95 tracked GTV V95 with Spearman ρ = 0.979. Boundary findings were stable across the threshold sweep. **Conclusions.** A closed-form heat-kernel structural prior captures more future-lesion volume than the prescription-dose envelope on a 43-patient brain-metastasis SRS cohort with patient-specific RTDOSE. The advantage is preserved across fractionation schedules. The findings are appropriate for retrospective audit and to motivate prospective adaptive-re-treatment evaluation, but the analysis does **not** include toxicity outcomes and **cannot** inform any prospective dose-escalation proposal in isolation.
...
This work is sole-authored. S.K.I. performed all CRediT contributor roles — Conceptualization, Methodology, Software, Validation, Formal analysis, Investigation, Data curation, Writing (original draft), Writing (review and editing), Visualization, and Project administration. The author has read and agreed to the published version of the manuscript.
